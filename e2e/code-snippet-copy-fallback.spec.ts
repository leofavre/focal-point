...
/**
 * Code snippet copy with Clipboard API unavailable (fallback path).
 * Plan: e2e/code-snippet-copy.plan.md (test 1.2)
 */
testWithFixtures.describe("Code snippet copy – Clipboard API unavailable", () => {
  testWithFixtures.use({ permissions: ["clipboard-read", "clipboard-write"] });

  testWithFixtures(
    "copies code snippet with correct object-position when Clipboard API is unavailable",
    async ({ pageWithoutClipboard: page }) => {
      await page.goto("/");
